...
screenGui.Name = "SkillCheckCompleteGUI"
screenGui.Parent = player.PlayerGui
...
titleLabel.Position = UDim2.new(0, 0, 0, 0)
titleLabel.BackgroundTransparency = 1
titleLabel.Text = "🎯 Авто-завершение после скилл чека"
titleLabel.TextColor3 = Color3.new(1, 1, 1)
titleLabel.TextScaled = true
titleLabel.Font = Enum.Font.SourceSansBold
...
statusLabel.BackgroundTransparency = 1
statusLabel.Text = "Ожидание скилл чека..."
statusLabel.TextColor3 = Color3.new(0.8, 0.8, 0.8)
statusLabel.TextScaled = true
statusLabel.Font = Enum.Font.SourceSans
...
-- Функция поиска активного генератора (тот, который ремонтируется)
local function findActiveGenerator()
    local char = player.Character
    ...
    -- Поиск ближайшего генератора с Panel.Progress
    local closest = nil
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj.Name:lower():find("generator") and obj:IsA("Model") then
            ...
            if panel and panel:FindFirstChild("Progress") then
                local distance
                if obj.PrimaryPart then
                    distance = (obj.PrimaryPart.Position - hrp.Position).Magnitude
                else
                    distance = (obj:GetModelCFrame().Position - hrp.Position).Magnitude
                end
                
                -- Ищем генератор в радиусе 8 единиц (чуть больше InteractDistance = 4)
                if distance < 8 and distance < closestDistance then
                    closest = obj
...
-- Функция мгновенного завершения генератора
local function instantCompleteGenerator(generator)
    ...
    print("⚡ Мгновенное завершение генератора: " .. generator.Name)
    print("   📊 Было: " .. tostring(progress.Value))
    ...
    pcall(function()
        ...
        if progress.Value == 1000 then
            success = true
            print("   ✅ Прогресс установлен на 1000!")
        end
    end)
    
    -- Метод 2: Быстрое увеличение
    if not success then
        pcall(function()
            local currentValue = progress.Value
            for i = currentValue, 1000, 100 do
                progress.Value = i
                wait(0.001)
            end
            progress.Value = 1000
            success = progress.Value == 1000
        end)
    end
    
    -- Метод 3: Через атрибуты
    if not success then
        pcall(function()
            panel:SetAttribute("Progress", 1000)
            progress:SetAttribute("Value", 1000)
            generator:SetAttribute("Completed", true)
            generator:SetAttribute("Repaired", true)
        end)
    end
    
    -- Метод 4: Попытка через RemoteEvents
    pcall(function()
        local remoteEvents = ReplicatedStorage:FindFirstChild("RemoteEvents")
        if remoteEvents then
            for _, remote in pairs(remoteEvents:GetDescendants()) do
                if remote:IsA("RemoteEvent") and 
                   (remote.Name:lower():find("repair") or remote.Name:lower():find("complete")) then
                    remote:FireServer("complete", generator)
                    remote:FireServer(generator, 1000)
                end
            end
        end
    end)
    
    print("   📊 Стало: " .. tostring(progress.Value))
    return success or progress.Value >= 1000
end

-- Основная функция обработки скилл чеков
local function setupSkillCheckHandler()
    print("🎯 Настройка обработчика скилл чеков...")
    
    local remoteEvents = ReplicatedStorage:FindFirstChild("RemoteEvents")
    if not remoteEvents then
        print("❌ RemoteEvents не найден!")
        statusLabel.Text = "RemoteEvents не найден"
        return
    end
    
    local skillCheckResult = remoteEvents:FindFirstChild("SkillCheckResult")
    if not skillCheckResult then
        print("❌ SkillCheckResult не найден!")
        statusLabel.Text = "SkillCheckResult не найден"
        return
    end
    
    print("✅ SkillCheckResult найден!")
    statusLabel.Text = "Система активна"
    
    -- Подключаемся к событию скилл чека
    skillCheckConnection = skillCheckResult.Event:Connect(function(actionType, result)
        local currentTime = tick()
        
        print("🎯 Скилл чек получен:")
        print("   🏷️ Тип: " .. tostring(actionType))
        print("   📊 Результат: " .. tostring(result))
        
        -- Проверяем, что это скилл чек ремонта и он успешен
        if actionType == "Repair" and result == "Success" then
            -- Защита от спама (не чаще раза в секунду)
            if currentTime - lastSkillCheckTime < 1 then
                print("   ⏰ Слишком быстро, пропускаем")
                return
            end
            
            lastSkillCheckTime = currentTime
            
            print("   ✅ Успешный скилл чек ремонта!")
            statusLabel.Text = "Скилл чек успешен! Завершаем..."
            statusLabel.TextColor3 = Color3.new(0, 1, 0)
            
            -- Ищем активный генератор
            local generator, distance = findActiveGenerator()
            
            if generator then
                print("   🎯 Найден активный генератор: " .. generator.Name)
                print("   📍 Расстояние: " .. math.floor(distance))
                
                -- Небольшая задержка для реалистичности
                wait(0.1)
                
                -- Мгновенно завершаем генератор
                local success = instantCompleteGenerator(generator)
                
                if success then
                    print("   🎉 ГЕНЕРАТОР ЗАВЕРШЕН!")
                    statusLabel.Text = generator.Name .. " завершен!"
                    
                    -- Визуальная обратная связь
                    statusFrame.BackgroundColor3 = Color3.new(0, 0.3, 0)
                    wait(2)
                    statusFrame.BackgroundColor3 = Color3.new(0.1, 0.1, 0.1)
                    statusLabel.TextColor3 = Color3.new(0.8, 0.8, 0.8)
                    statusLabel.Text = "Ожидание скилл чека..."
                else
                    print("   ❌ Не удалось завершить генератор")
                    statusLabel.Text = "Ошибка завершения"
                ...
                print("   ❌ Активный генератор не найден")
                statusLabel.Text = "Генератор не найден"
        ...
        elseif actionType == "Repair" and result == "Fail" then
            print("   ❌ Скилл чек провален")
            statusLabel.Text = "Скилл чек провален"
            statusLabel.TextColor3 = Color3.new(1, 0, 0)
            wait(1)
            statusLabel.TextColor3 = Color3.new(0.8, 0.8, 0.8)
            statusLabel.Text = "Ожидание скилл чека..."
    ...
    print("🎯 Обработчик скилл чеков активен!")
end

-- Дополнительный мониторинг для отладки
local function startDebugMonitoring()
    RunService.Heartbeat:Connect(function()
        local generator, distance = findActiveGenerator()
        if generator ~= currentGenerator then
            currentGenerator = generator
            if generator then
                print("🔧 Активный генератор: " .. generator.Name .. " (Расстояние: " .. math.floor(distance) .. ")")
                local panel = generator:FindFirstChild("Panel")
                if panel and panel:FindFirstChild("Progress") then
                    print("   📊 Текущий прогресс: " .. panel.Progress.Value .. "/1000")
                end
            end
        end
    end)
...
-- Запуск системы
setupSkillCheckHandler()
startDebugMonitoring()

print("🎯 ==================== СИСТЕМА ЗАПУЩЕНА ====================")
print("✅ Автоматическое завершение генератора после успешного скилл чека")
print("📋 Как работает:")
print("   1. Проходите скилл чек вручную")
print("   2. При успехе генератор автоматически завершается")
print("   3. Progress.Value устанавливается на 1000")
